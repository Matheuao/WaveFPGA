...
--  Version      : 1.0
--  Description:
--  Computes the direct transform of the NDWT
--
--  License:
...
signal temp_b0: signed (W1-1 downto 0) :=( others=>'0');
signal temp_output_high:signed(W1-1 downto 0):=( others=>'0');
...
gen_Shared_multipliers: if optimization = Shared_multipliers generate
	...
	sum :for i in coefficient_size-1 downto 1 generate -- FIR transposed form
		...
		output_lowpass_coefficients: reg generic map(W1=>W1) 
			port map(reg_in=>a(0)((W1+W2)-2 downto ((W1+W2)-2) - 15),
					 load=>load,
					 reset=>reset,
					 clk=>clk,
